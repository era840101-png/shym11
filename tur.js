...
function updateClock() {
 ...
 var map = null;
var places = [
    { name: "Абай ескерткіші", lat: 43.238949, lng: 76.889709, img: "" },
    { name: "Республика алаңы", lat: 43.242000, lng: 76.928000, img: "" },
    { name: "Көктөбе", lat: 43.233500, lng: 76.975700, img: "" },
    { name: "Медеу", lat: 43.157500, lng: 77.058300, img: "" }
];

function toggleMap() {
    var container = document.getElementById('map-container1');
    var btn = document.getElementById('mainBtn1');

    if (map !== null) {
        map.remove();
        map = null;
        container.style.display = 'none';
        btn.innerText = '🛣 Маршрутты ашу';
        btn.className = '';
    } else {
        container.style.display = 'block';
        btn.innerText = '❌ Картаны жабу';
        btn.className = 'close-btn';

        // Ескерту: 'map-leaflet' идентификаторы HTML-дегі жаңа ID-мен сәйкес болуы керек
        map = L.map('map-leaflet').setView([43.238949, 76.889709], 12);
        L.tileLayer('https://{s}.tile.openstreetmap.org/{z}/{x}/{y}.png').addTo(map);

        places.forEach(function(p) {
            L.marker([p.lat, p.lng]).addTo(map)
                .bindPopup(`<div class="popup-content"><h3>${p.name}</h3><img src="${p.img}"></div>`);
        });

        L.Routing.control({
            waypoints: places.map(p => L.latLng(p.lat, p.lng)),
            lineOptions: { styles: [{ color: '#0b6efd', weight: 6 }] },
            createMarker: () => null,
            addWaypoints: false
        }).addTo(map);
        
        // Карта көлемін дұрыстау үшін
